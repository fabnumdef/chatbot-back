...
export class InboxService {
  ...
  findAvgQuestPerVisitor(filters: StatsFilterDto): Promise<string> {
    ...
    const query = this._inboxesRepository
      .createQueryBuilder('inbox')
      .select(
        'ROUND(greatest(count(*), 1) * 1.0 / greatest(count(distinct inbox.sender_id), 1), 2) as averageQuestions',
      );
    if (startDate) {
      query.where(`DATE(to_timestamp(inbox.timestamp)) >= '${startDate}'`);
    }
    ...
    const query = this._inboxesRepository
      ...
      .select(
        `100 * (SELECT COUNT(inbox.id) from inbox WHERE inbox.confidence >= ${confidence.toString(
          10,
        )} ${additionnalWhereClause})/greatest(COUNT(inbox.id), 1) as ratioResponseOk`,
      );
    if (startDate) {
      query.where(`DATE(to_timestamp(inbox.timestamp)) >= '${startDate}'`);
    }
  ...
  async findRatioFeedback(
    ...
    const query = this._inboxesRepository
      .createQueryBuilder('inbox')
      .select(
        `100 * (SELECT COUNT(inbox.id) from inbox WHERE inbox.feedback_status = '${feedbackStatus}' ${additionnalWhereClause})/greatest(COUNT(inbox.id), 1)`,
        'ratioFeedback',
      );
    if (startDate) {
      query.where(`DATE(to_timestamp(inbox.timestamp)) >= '${startDate}'`);
