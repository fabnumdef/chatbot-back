...
export class RasaService {
  ...
  private _chatbotTemplateDir = path.resolve(
    __dirname,
    '../../../chatbot-template',
  );
  ...
  private async _intentsToRasa(intents: Intent[]) {
    ...
    for (const intent of intents) {
      ...
      if (intent.id === 'nlu_fallback') {
        const config: ChatbotConfig =
          await this._configService.getChatbotConfig();
    ...
    fs.writeFileSync(
      `${this._chatbotTemplateDir}/domain.yml`,
      yaml.dump(domain),
      'utf8',
    );
    fs.writeFileSync(
      `${this._chatbotTemplateDir}/data/nlu.yml`,
      yaml.dump({ version: '3.1', nlu }),
      'utf8',
    );
    fs.writeFileSync(
      `${this._chatbotTemplateDir}/data/rules.yml`,
      yaml.dump({
        version: '3.1',
        rules,
      }),
   ...
   * @param intent
   */
  private _generateDomainUtter(intent: Intent): {
      ...
      switch (response.response_type) {
        case ResponseType.text:
          responses[`utter_${intent.id}_${index}`] = [
            { text: this._cleanStringForYaml(response.response, false) },
          ];
          break;
  ...
  private async _deleteOldModels() {
    try {
      ...
      await execShellCommand(
        "rm `ls -t | awk 'NR>5'`",
        path.resolve(this._chatbotTemplateDir, 'models'),
      ).then((res) => {
        this._logger.log(res);
      });
    } catch (e) {
